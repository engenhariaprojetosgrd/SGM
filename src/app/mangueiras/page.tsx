...
import { EQUIPMENT_LIST, SYSTEM_LIST, SUPPLIER_LIST, HOSE_TYPES } from '@/types'
import { fmt } from '@/lib/calculations'

const empty: Partial<Hose> = { equip:'', system:'', position:'', supplier:'', hose_type:'Genuína (OEM)', part_number:'', unit_cost:0, install_hours:0, notes:'', status:'active' }
...
export default function MangueirasPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    setSaving(true)
    const payload = { ...form, id: nextId(hoses), status: 'active' } as Hose
    const { error } = await supabase.from('hoses').insert(payload)
    if (error) { setMsg({ text: 'Erro: ' + error.message, type: 'err' }) }
    else { setMsg({ text: 'Mangueira cadastrada!', type: 'ok' }); setForm(empty); load() }
    setSaving(false)
        ...
        <form onSubmit={submit}>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div><label className="lbl">Equipamento *</label>
              <select className="inp" required value={form.equip} onChange={e=>setForm(p=>({...p,equip:e.target.value}))}>
            ...
            <div><label className="lbl">Posição / Local *</label>
            ...
            {form.supplier==='__other__' && (
              <div><label className="lbl">Nome do Fornecedor *</label>
                <input className="inp" required placeholder="Nome" onChange={e=>setForm(p=>({...p,supplier:e.target.value}))} />
              </div>
            )}
            ...
            <div><label className="lbl">Vida Útil Esperada (h)</label>
            ...
            <div className="col-span-2 md:col-span-3"><label className="lbl">Observações</label>
              <textarea className="inp h-16 resize-none" value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} />
            </div>
          </div>
          <div className="mt-4">
            <button className="btn btn-primary" type="submit" disabled={saving}>{saving?'Salvando...':'✓ Cadastrar Mangueira'}</button>
          </div>
        </form>
      </div>
      ...
      {loading ? <div className="text-center py-8 text-gray-400">Carregando...</div> : (
        <>
          <div className="card">
            ...
            <div className="overflow-x-auto">
              <table className="tbl w-full">
                <thead><tr><th>ID</th><th>Equipamento</th><th>Sistema</th><th>Posição</th><th>Fornecedor</th><th>Tipo</th><th>Custo</th><th>Instalação</th><th>Horímetro</th></tr></thead>
                <tbody>
          ...
          <div className="card">
            ...
            <div className="overflow-x-auto">
              <table className="tbl w-full">
                <thead><tr><th>ID</th><th>Equipamento</th><th>Sistema</th><th>Posição</th><th>Fornecedor</th><th>Custo</th><th>Instalação</th></tr></thead>
                <tbody>
                  ...
                  {replaced.map(h=>(
                    <tr key={h.id} className="opacity-60">
                      ...
                      <td className="font-bold">{h.equip}</td><td>{h.system}</td><td>{h.position}</td>
                      <td><SupplierBadge s={h.supplier}/></td>
                      <td>{fmt(h.unit_cost)}</td><td>{h.install_date??'—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
